...
async function loadDynamics({ announce = false } = {}) {
  ...
  try {
    ...
    localStorage.setItem('chenlijun:dynamicsCache', JSON.stringify(data));
    if (announce) showToast('已完成同步');
  } catch (error) {
    let cached = null;
    try { cached = JSON.parse(localStorage.getItem('chenlijun:dynamicsCache') || 'null'); } catch {}
...
$('#themeBtn').addEventListener('click', () => {
  ...
  localStorage.setItem('chenlijun:theme', document.body.classList.contains('dark') ? 'dark' : 'light');
});
if (localStorage.getItem('chenlijun:theme') === 'dark') document.body.classList.add('dark');

$('#favBtn').addEventListener('click', () => {
  const on = localStorage.getItem('chenlijun:fav') !== '1';
  localStorage.setItem('chenlijun:fav', on ? '1' : '0');
  $('#favBtn').textContent = on ? '♥ 已收藏' : '♡ 收藏';
  showToast(on ? '已收藏到本机' : '已取消收藏');
});
if (localStorage.getItem('chenlijun:fav') === '1') $('#favBtn').textContent = '♥ 已收藏';
...
let checkCount = Number(localStorage.getItem('chenlijun:check') || 0);
...
function renderQuizResult(score) {
  localStorage.setItem('chenlijun:quizBest', String(Math.max(score, Number(localStorage.getItem('chenlijun:quizBest') || 0))));
  modalBody.innerHTML = `
...
$$('[data-modal]').forEach(button => button.addEventListener('click', () => {
  ...
  if (type === 'checkin') {
    checkCount += 1;
    localStorage.setItem('chenlijun:check', String(checkCount));
    $('#checkCount').textContent = `${checkCount} 次`;
  ...
  if (type === 'chenlijun:note') {
    modalBody.innerHTML = `<h2>写一封云笺</h2><textarea id="noteArea" maxlength="500" placeholder="写下你对舞台、角色或越剧的感受…">${escapeHTML(localStorage.getItem('chenlijun:note') || '')}</textarea><div class="form-actions"><button class="secondary-action" id="cancelNote">取消</button><button class="action" id="saveNote">保存到本机</button></div>`;
    openModal();
    $('#cancelNote').addEventListener('click', closeModal);
    $('#saveNote').addEventListener('click', () => {
      localStorage.setItem('chenlijun:note', $('#noteArea').value.trim());
      showToast('云笺已保存到本机');
      closeModal();
    });
    return;
...
function renderRemoteGallery(){
  ...
  grid.querySelectorAll('img').forEach(img=>img.addEventListener('error',()=>{if(img.dataset.fallbackApplied)return;img.dataset.fallbackApplied='1';img.src='./assets/hero-chen-lijun.webp';img.closest('.gallery-image-wrap')?.classList.add('using-fallback');},{once:true}));
...
function openGallery(index){
  ...
  big.addEventListener('error',()=>{if(big.dataset.fallbackApplied)return;big.dataset.fallbackApplied='1';big.src='./assets/hero-chen-lijun.webp';big.parentElement.classList.add('using-fallback');},{once:true});
...
async function loadIntel(announce=false){
 ...
 let data=null,mode='';
 try{const r=await fetch(`/api/news?actor=chen-lijun&v=${Date.now()}`,{cache:'no-store',headers:{Accept:'application/json'}});if(!r.ok)throw new Error();const d=await r.json();if(!d.items?.length)throw new Error();data=d;mode='实时聚合';}catch{}
 ...
 if(!data){try{const d=JSON.parse(localStorage.getItem('chenlijun:intel-cache')||'null');if(d?.items?.length){data=d;mode='本机缓存';}}catch{}}
 if(!data){data=intelFallback;mode='基础资料';}
 intelPayload=data;localStorage.setItem('chenlijun:intel-cache',JSON.stringify(data));renderIntel();$('#intelStatus').innerHTML=`已载入 <b>${escapeHTML(mode)}</b> · 最近检查 ${escapeHTML(formatTime(data.checkedAt||data.updatedAt))}`;btn.disabled=false;if(announce)showToast(`情报站已刷新：${mode}`);
}

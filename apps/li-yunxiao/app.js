...
async function loadIntel(announce=false){
 ...
 let data=null,mode='';
 try{const r=await fetch(`/api/news?actor=li-yunxiao&v=${Date.now()}`,{cache:'no-store',headers:{Accept:'application/json'}});if(!r.ok)throw new Error();const d=await r.json();if(!d.items?.length)throw new Error();data=d;mode='实时聚合';}catch{}
